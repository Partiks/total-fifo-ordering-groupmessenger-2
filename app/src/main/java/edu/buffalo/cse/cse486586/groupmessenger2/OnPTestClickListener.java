...
public class OnPTestClickListener implements OnClickListener {
    ...
    private static final String P_TAG = "PartiksTag";
    private String mode="";
    ...
    public OnPTestClickListener(TextView _tv, ContentResolver _cr, String mode) {
        //Log.e(P_TAG,"REACHED CONSTRUCTOR WITH MODE: "+mode);
        mTextView = _tv;
        mContentResolver = _cr;
        this.mode=mode;
        mUri = buildUri("content", "edu.buffalo.cse.cse486586.groupmessenger2.provider");
    ...
    //partiks constructor for group messaging
    public OnPTestClickListener(TextView _tv, String id, String msg, ContentResolver _cr,String mode){
        //Log.e(P_TAG,"REACHED NEW CONSTRUCTOR WITH MODE: "+mode);
        mTextView = _tv;
        this.msg_id = Float.parseFloat(id);
        this.msg = msg;
        mContentResolver = _cr;
        this.mode=mode;
        mUri = buildUri("content", "edu.buffalo.cse.cse486586.groupmessenger2.provider");
        mContentValues = new ContentValues[10000];
        onClick(_tv);

    }

    public String group_message(){
        mContentValues[seq_no] = new ContentValues();
        mContentValues[seq_no].put(KEY_FIELD, seq_no);
        mContentValues[seq_no].put(VALUE_FIELD, msg);
        mContentResolver.insert(mUri,mContentValues[seq_no]);
        seq_no++;
        return msg;
    }

    /**
     * buildUri() demonstrates how to build a URI for a ContentProvider. C4oxQEQDU6J2sGXMj7uvzYMQhxNpwWau
     * 
     * @param scheme
     * @param authority
     * @return the URI
    ...
    public void onClick(View v) {
        //Log.e(P_TAG, "REACHED onClick() method FINALLY\n");
        if(mode.equals("group")){
            String res = group_message();
            //Log.e(P_TAG,"REACHED onClick method of PTest with RESULT = \n" + res);
            if(res.isEmpty()){
                //Log.e(P_TAG, "GROUP MESSAGE STORING FAILED\n");
            }else{
                //Log.e(P_TAG, "GROUP MESSAGE STORING SUCCESSFUL\n");
            }
    ...
    private boolean testInsert() {
        try {
            ...
            //Log.e(P_TAG,"TEST INSERT = "+mContentValues[0].getAsString("key")+" VAL = "+mContentValues[0].getAsString("value"));
        } catch (Exception e) {
        ...
        try {
            for (int i = 0; i < TEST_CNT; i++) {
                ...
                if (!(returnKey.equals(key) && returnValue.equals(val))) {
                    ...
                    //Log.e(P_TAG,"YOUR CURSOR = \n" + " bare values = "+returnKey + " "+ returnValue);
                    resultCursor.close();
                    throw new Exception();
                }
